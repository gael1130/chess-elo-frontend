...
import { FSRSStatus } from "@/data/types";
...
export function ChessPuzzle({
  ...
  const [status, setStatus] = useState<
    "initial" | "correct" | "incorrect" | "solved"
  >("initial");
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      try {
        ...
        console.log(
          `Made opponent's move: ${puzzle.opponentMove.from} to ${puzzle.opponentMove.to}`
        );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center space-y-2 sm:space-y-0">
        ...
        {attemptHistory && (
          <div className="text-xs text-muted-foreground">
            {attemptHistory.attempts_count &&
            attemptHistory.attempts_count > 0 ? (
              <div className="flex items-center space-x-2">
                <span className="px-2 py-0.5 bg-blue-500/20 text-blue-500 rounded-full">
                  Reviewed {attemptHistory.attempts_count} time
                  {attemptHistory.attempts_count !== 1 ? "s" : ""}
                </span>
                <span
                  title={`Difficulty: ${
                    attemptHistory?.difficulty
                      ? attemptHistory.difficulty.toFixed(1)
                      : "N/A"
                  }`}
                >
                  D:{" "}
                ...
                <span
                  title={`Stability: ${
                    attemptHistory?.stability
                      ? attemptHistory.stability.toFixed(1)
                      : "N/A"
                  } days`}
                >
                  S:{" "}
                ...
                <span
                  title={`Retrievability: ${
                    attemptHistory?.retrievability
                      ? (attemptHistory.retrievability * 100).toFixed(0)
                      : "N/A"
                  }%`}
                >
                  R:{" "}
        ...
        <div className="flex justify-center space-x-4 text-xs text-muted-foreground">
          {triesCount > 0 && (
            <span className="text-amber-500">
              Incorrect attempts: {triesCount}
            </span>
          )}
          {hintUsed && <span className="text-blue-500">Hint used</span>}
        </div>
      )}
    </div>
  );
